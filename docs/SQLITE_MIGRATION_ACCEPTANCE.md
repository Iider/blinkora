...
- **M1 唯一发布阻断是本机 `BLINKORA_SECRET` 处置。** 本轮隔离恢复排障的一条过宽检查命令在工具输出中回显了现用值；它没有写入仓库或正式证据文件，但仍应轮换或由用户明确接受风险。S3 AccessKey 已由用户明确安排在正式使用时自行轮换，不再作为当前开发阻断。
...
2026-07-16，冻结候选的 macOS 本地工程门禁通过，M1 状态仍为 **技术通过，待安全收口**，唯一安全阻断是本机 `BLINKORA_SECRET` 的处置。S3 AccessKey 由用户在正式使用时自行轮换。
