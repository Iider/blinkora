...
| 可重复 macOS 原生浏览器 smoke（本轮） | `bun run smoke:browser-local` 从空目录构建独立原生 release 和 SQLite 服务，不使用 Docker、不访问常驻 `6676`；以本机 Chrome 在桌面与 390×844 视口完成注册/登录、工作区创建/自动切换/重载后回显、闪念/笔记/待办创建、笔记编辑、待办完成/恢复、置顶、归档/恢复、回收站/恢复、评论新增、全局搜索、根目录和嵌套资源目录创建。它对每个本机 4xx/5xx、console error 与 page error 失败；结束前断言三种类型各一条、编辑正文与所选 Workspace 已保存、编辑笔记存在历史与操作日志、每张笔记历史版本从 1 连续递增、两条目录占位记录已落盘、`integrity_check=ok`、`foreign_key_check=0` 及 Workspace/历史/评论/附件/标签/操作日志关联 orphan 为 0。实跑通过后临时服务、SQLite 和构建目录均已删除，6676 保持健康。该回归还发现并修复“编辑笔记会写回闪念类型”：编辑器现在持有被编辑笔记的类型，不再依赖可能变化的全局选择。 |
