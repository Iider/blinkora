...
- 候选功能提交：`1b15eae`（SQLite 运行时替换及全部既有验收门禁；本轮新增公共配置脱敏、S3 设置与图片预览修复、真实 S3 浏览器 smoke、编辑器附件事务删除、共享资源保护，以及附件变化的操作日志记录）。
...
| 真实云 S3 | 已在阿里云 OSS 北京区域执行 `bun run smoke:s3-local`，全程不使用 Docker。最近一次只使用全新的 `codex-sqlite-qa-20260716-attachment-tx-pass2/smoke-*` 随机子前缀，不列举、不读取也不复用现有 `blinkora/`、`blinkora_local/` 对象。API smoke 完成配置校验、上传、逐字节读取、移动、再次读取、删除、删除后 404 和回退本地；随后 Chrome 浏览器真实验证无效配置失败且保持本地、表单值保留、有效配置启用、PNG 上传/编辑器预览/卡片预览、字节一致读取、编辑器事务删除、对象 404 及恢复本地存储。最终 `integrity_check=ok`、`foreign_key_check=0`；临时服务、SQLite、构建目录和已知测试对象均已清理，凭据未写入日志或仓库。 |
...
| PostgreSQL 残留防回归 | `verify:rust` 会执行 `scripts/verify-sqlite-runtime-residuals.mjs`：检查 17 个部署文档、模板与 Compose 文件不含 PostgreSQL URL、`psql`、旧容器名、默认端口或运行时环境变量；检查 31 个正式运行时源文件不含 PostgreSQL 专用 backend 或 SQL 语法，并要求 Compose 只定义 `web` 服务。最新 `verify:rust` 已通过 58 个服务端测试和 6 个迁移工具测试。 |
...
- 桌面和移动端的自动化核心浏览器 smoke 已覆盖登录/两步验证、Workspace 创建/切换/重命名/默认保护/单卡和批量移动/当前 Workspace 级联删除、三类笔记新建/编辑/历史/类型往返、自定义属性类型/非法值/清空/Markdown 链接/卡背回显、每日回顾、标签树、附件上传/下载字节校验/编辑器事务删除/共享资源保护/移动/重命名/单项与多项删除、引用、置顶/归档/回收、评论树、组合筛选、分页、操作日志、字体、导入导出、Agent 令牌和资源目录。真实 S3 浏览器 smoke 也已覆盖设置成功与失败、PNG 预览、字节读取和编辑删除。完整人工清单仍未 100% 执行：富文本细节、复制、全屏、主题和滚动等视觉/手势状态，音频/视频预览与相机/麦克风路径，多引用组合边界，以及 S3 卡片彻底删除时同步级联对象的 UI 选择仍需逐项验证。
...
在剩余人工浏览器清单和生产数据切换补齐并记录结果前，本次数据库替换不能标记为“全部验收通过”。飞牛版本另按上述平台门禁验收。
