...
- 当前交付源码提交：`899c4b9`（在 M1 冻结点之后加入 M2 切换保护、本机认证密钥轮换、TypeScript 契约修复及正式类型门禁）。后续的 Linux 便携交付和飞牛 SQLite unit 另有本次实际部署记录。
...
| 构建与部署 | M1 冻结候选的 `bun run build:web --force`、`bun run verify:rust` 和 macOS 原生 release 构建均通过；`bun run deploy:local update` 再次完成同一 Web/native 构建并更新 `launchd` 服务。当前交付源码点 `899c4b9` 在 TypeScript 修复后重新通过 `bun run typecheck`、`bun run build:web --force` 和 `bun run verify:rust`；这轮没有再次更新常驻 `launchd` release，不把源码门禁写成本机部署更新。既有 Linux arm64/amd64 与单 Web 容器运行时证据来自此前候选，当前产品代码改动后没有重跑：本机 Homebrew Rust 不提供 Linux musl target，而本次明确不使用 Docker，因此 `bun run build:rust-release` 的 Linux 阶段是平台范围跳过，不写成当前候选通过。Docker 资源最终只读复核为 0 镜像、0 容器、0 卷、0 Build Cache，默认网络之外为 0。 |
...
| PostgreSQL 残留防回归 | `verify:rust` 会执行 `scripts/verify-sqlite-runtime-residuals.mjs`：检查部署文档、模板与 Compose 文件不含 PostgreSQL URL、`psql`、旧容器名、默认端口或运行时环境变量；检查正式运行时源文件不含 PostgreSQL 专用 backend 或 SQL 语法，并要求 Compose 只定义 `web` 服务。当前交付源码点最新实跑通过 64 个服务端测试和 6 个迁移工具测试。 |
