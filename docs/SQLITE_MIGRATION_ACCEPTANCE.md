...
| 可重复 macOS 原生浏览器 smoke（本轮） | 新增 `bun run smoke:browser-local`。脚本从空目录构建独立原生 release 和 SQLite 服务，不使用 Docker、不访问常驻 `6676`；以本机 Chrome 在桌面与 390×844 视口完成注册/登录、闪念/笔记/待办创建、笔记编辑、全局搜索和资源页渲染。它对每个本机 4xx/5xx、console error 与 page error 失败；结束前断言三种类型各一条、历史 `version=1`、编辑正文已保存、`integrity_check=ok`、`foreign_key_check=0` 及 Workspace/历史/评论/附件/标签关联 orphan 为 0。实跑通过后临时服务、SQLite 和构建目录均已删除，6676 保持健康。该回归还发现并修复“编辑笔记会写回闪念类型”：编辑器现在持有被编辑笔记的类型，不再依赖可能变化的全局选择。 |
