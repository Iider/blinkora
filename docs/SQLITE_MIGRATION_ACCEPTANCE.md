...
- M1 收口源码提交：`899c4b9`（在 M1 冻结点之后加入 M2 切换保护、本机认证密钥轮换、TypeScript 契约修复及正式类型门禁）。后续的 Linux 便携交付和飞牛 SQLite unit 另有本次实际部署记录。
- 当前部署范围：2026-07-16 已将 macOS 本机 SQLite 的一致物理快照恢复到飞牛新目录，并启用飞牛 SQLite 服务作为唯一主服务。旧 PostgreSQL 安装与数据保留但不运行；这不是旧 M2 运行手册中“PostgreSQL 直迁 macOS”的历史路径，旧记录仍仅用于追溯当时的预演。
...
| 构建与部署 | M1 冻结候选的 `bun run build:web --force`、`bun run verify:rust` 和 macOS 原生 release 构建均通过；`bun run deploy:local update` 再次完成同一 Web/native 构建并更新 `launchd` 服务。M1 收口源码点 `899c4b9` 在 TypeScript 修复后重新通过 `bun run typecheck`、`bun run build:web --force` 和 `bun run verify:rust`；该轮没有再次更新常驻 `launchd` release。2026-07-17 的原生单二进制收敛重新通过 `bun run verify:rust`，共 65 个服务端测试和 6 个迁移工具测试；`bun run build:linux-headless` 生成静态 PIE x86_64 Linux 文件，SHA-256 为 `ab31aa7c67113516d40ac59157cadc9cc678eab23e1221f79c70787c2f3f934e`。本机构建因缺少 Linux musl target 使用了临时 Docker builder，复制产物后构建容器和镜像均已删除；目标服务器始终直接运行二进制，本轮没有更新 macOS 或飞牛常驻服务。 |
...
| PostgreSQL 残留防回归 | `verify:rust` 会执行 `scripts/verify-sqlite-runtime-residuals.mjs`：检查当前运行文档和模板不含 PostgreSQL URL、`psql`、旧容器名、默认端口或运行时环境变量；检查正式运行时源文件不含 PostgreSQL 专用 backend 或 SQL 语法；同时要求已退役的 Compose、运行时镜像和容器环境模板保持不存在。当前统一验收实跑通过 65 个服务端测试和 6 个迁移工具测试。 |
...
- **M1 凭据收口已完成。** 本机 `BLINKORA_SECRET` 已轮换为新的 32 字节随机值，服务重启健康，旧 JWT/API token 已拒绝，数据库完整性、外键和环境权限保持正常；新旧值均未进入仓库或验收日志。S3 AccessKey 由用户在正式使用时自行轮换，不再作为当前开发阻断。
