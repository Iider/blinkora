...
| macOS 本机持久化 | 在隔离用户目录中实际执行 `install → update → smoke:rust → smoke:agent → stop → start → uninstall`，全程不启动 Docker。重启后原账号能登录、笔记数不变；卸载后 `blinkora.sqlite3` 保留，`integrity_check=ok`、`foreign_key_check` 为 0。2026-07-16 又将冻结候选 `84c40ff` 更新到默认本机目录：`com.blinkora.local` 为 running，`/health=200`、首页 `200`，部署 `index.html` 与本轮构建一致，最近 80 行 stdout/stderr 错误标记为 0；原 `DATA_DIR` 未替换。常驻库 `integrity_check=ok`、外键 0 行，Workspace、标签、引用、评论、附件、历史和日志 orphan 均为 0；`~/.blinkora`、`local`、`data` 为 `0700`，环境文件、数据库、WAL 和 SHM 为 `0600`。 |
...
- 本机切换：先对原本机数据执行离线物理备份，再在同一文件系统原子替换 `data` 和运行环境文件；旧本机目录及环境文件保留为回滚点。切换后 14 表计数与源快照逐项一致，`integrity_check=ok`、`foreign_key_check=0`、显式 orphan 合计为 0，首页和 `/health` 均返回 200。
...
- 清理结果：临时 PostgreSQL、隔离验证库、恢复演练副本、构建缓存和临时登录/token 文件均已删除。保留项只有源快照、无敏感输出的校验证据、切换前后物理备份、旧本机回滚目录和正式本机数据；本机 Docker 容器、镜像和卷均为 0。

## 待验 / 阻断发布

- **M1 唯一发布阻断是凭据处置。** 对话中曾暴露真实 S3 AccessKey；代码库、命令输出和验收日志扫描未发现密钥，但公开过的密钥仍应视为已泄露。用户需要完成轮换，或明确接受临时风险并给出轮换期限。在此之前只能写“M1 技术门禁通过，待安全收口”，不能签署“M1 已交付”。
- **M2 最终停写切换尚未执行。** 真实数据的只读快照、14 表迁移、本机原子切换、凭据延续、物理备份和本机回滚预演已经通过；但飞牛在快照后仍可继续写入，所以本机只是 `2026-07-16T08:46:21Z` 的点时间副本。最终维护窗口仍需停掉 Web/MCP/Agent 写入口，重新取得最终快照并校验增量，再决定主服务角色，不能把这次副本写成生产切换完成。
...
- **真实数据浏览器 smoke 尚未执行。** 正式本机副本已通过账号、API token、Workspace token、Workspace/笔记列表和健康接口的只读验证；最小写入仅在隔离克隆上完成。为避免触碰现有 S3，本轮没有在真实副本上执行附件页面、完整浏览器清单或 MCP 写入；这些仍属于最终切换门禁。
...
2026-07-16，冻结候选的 macOS 本地工程门禁通过，M1 状态仍为 **技术通过，待安全收口**，唯一安全阻断是已暴露 S3 AccessKey 的用户侧处置。

同日完成的 M2 只读副本迁移已经证明真实 14 表数据可无损落到本机 SQLite，原账号和令牌可继续使用，备份与本机回滚路径可用，飞牛和 S3 业务对象没有被改写。由于飞牛仍是主服务、没有最终停写快照、没有验证真实 S3 内容和完整真实数据浏览器/MCP 写入链，当前结论只能写 **M2 只读副本迁移通过，最终生产切换待维护窗口**，不能宣告原始数据库替换目标全部完成。
