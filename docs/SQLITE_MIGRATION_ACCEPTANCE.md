...
- 桌面和移动端核心浏览器 smoke 已完成，且已覆盖登录/两步验证、Workspace 创建/切换/单卡移动、三类笔记新建和编辑/历史、每日回顾、嵌套标签树父/子节点筛选/刷新后回显、附件上传/确认删除/重命名、引用、置顶/归档/回收/恢复、评论树新建/回复/编辑/两级删除、日期范围、附件/外链/Markdown 待办/无标签条件筛选及其重置（可持久筛选另验证刷新后回显）、操作日志正文筛选、隔离库中本地字体的选择/重载/恢复默认、闪念/笔记/待办、全部/归档/回收站列表的页码分页（第 2 页刷新/删除与越界页；回收站另覆盖确认后的彻底删除）、设置页空工作区的 Markdown 导出导入与全量 JSON 导出导入、全局搜索和资源目录创建/附件与目录重命名/嵌套/删除；完整数据浏览器清单仍未逐项执行。仍需覆盖附件与资源目录全部动作、其余高级筛选、设置/S3 以及 MCP/Agent 的全部 UI 路径。
