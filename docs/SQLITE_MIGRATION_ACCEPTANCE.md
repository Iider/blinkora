...
- 当前交付源码提交：`899c4b9`（在 M1 冻结点之后加入 M2 切换保护、本机认证密钥轮换、TypeScript 契约修复及正式类型门禁；没有执行 M2 最终生产切换）。后续纯文档提交不改变该源码标识。
...
2026-07-16，冻结候选的 macOS 本地工程门禁和凭据收口均已通过，M1 状态为 **本地软件交付完成，生产迁移待 M2**。S3 AccessKey 由用户在正式使用时自行轮换。
