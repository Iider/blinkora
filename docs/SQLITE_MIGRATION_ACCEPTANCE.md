...
- M1 交付平台：macOS arm64、本地文件系统、SQLite CLI 3.x、Rust 1.96，全程本机原生构建和 `launchd` 部署，不使用 Docker。历史差分、迁移和 Linux 构建证据曾使用隔离 PostgreSQL 14.23 与 OrbStack，但本轮没有连接飞牛或生产服务器。
...
- **M2 生产迁移尚未开始。** 生产数据仍在用户当前服务器。按用户要求，本轮没有连接、读取、停写、快照、迁移或修改该服务器，也没有读取其 S3 业务对象；14 表迁移、凭据延续、哈希和回滚证据来自隔离 PostgreSQL 夹具，不能冒充生产切换证据。只有用户另行安排维护窗口并明确授权后，才能执行真实快照、预演、切换和回滚演练。
...
2026-07-16，冻结候选的 macOS 本地工程门禁通过：功能、数据完整性、常驻部署、隔离 S3、浏览器/API/Agent、权限和清理证据齐全，生产服务和既有 S3 业务数据均未触碰。凭据风险尚未完成用户侧处置，因此 M1 状态为 **技术通过，待安全收口**；生产 PostgreSQL 数据切换仍属于 M2，原始数据库替换目标不能标记为“全部验收通过”。
