...
该命令会重新构建一个临时原生 release，在独立 SQLite 目录中完成桌面和移动登录、工作区创建并自动切换、三类笔记创建、笔记编辑/历史、全局搜索，以及根目录和嵌套资源目录创建。它检查浏览器 console、SQLite 完整性、外键、orphan 和工作区/目录记录落盘。它不访问 `6676` 的常驻服务，结束时会删除临时服务、数据和构建目录。
