...
- macOS 原生浏览器 smoke：`bun run smoke:browser-local` 会建立临时 release、SQLite 和服务，不使用 Docker，也拒绝访问常驻 `6676` 服务。它以本机 Chrome 跑桌面/390×844 登录、工作区创建/自动切换/重载后回显、三类笔记创建、每日回顾、闪念/笔记/待办编辑（各自历史；笔记另含标签/附件新增、删除与引用）、待办完成/恢复、置顶、归档/恢复、回收站/恢复、评论树新建/回复/编辑/两级删除、附件条件筛选/重置、页码分页（第 2 页刷新和删除后保持页码、越界页回首页）、单卡移动到默认工作区并回显评论树、全局笔记/附件搜索，以及资源目录创建/重命名/嵌套、附件移入目录后移回根目录、同前缀兄弟目录删除保护，要求 console error 与本机 4xx/5xx 均为 0；结束前检查 SQLite 完整性、外键、连续历史版本、编辑操作日志、迁移笔记的历史/评论/附件/标签工作区归属、已删除附件不存在、Workspace/历史/评论/附件/标签关联 orphan 和目录记录，并删除临时数据。
- 真实云 S3 smoke：在隔离验收实例中，将 `BLINKORA_BASE_URL`、测试账号、`BLINKORA_S3_SMOKE_ENDPOINT`、`BLINKORA_S3_SMOKE_REGION`、`BLINKORA_S3_SMOKE_BUCKET`、`BLINKORA_S3_SMOKE_ACCESS_KEY` 和 `BLINKORA_S3_SMOKE_SECRET_KEY` 写入权限为 `0600` 的本地环境文件；加载后执行 `BLINKORA_S3_SMOKE_ISOLATED=1 bun run smoke:s3`。该命令缺少任一变量或隔离确认会失败，覆盖真实 S3 配置、`/api/s3file/*` 上传读取、资源移动、对象删除和回退本地存储；不在生产实例运行。
