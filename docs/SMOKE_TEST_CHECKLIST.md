...
| [ ] | 自定义属性编辑 | 打开一张测试笔记全文，在底部属性区写入 `type: permanent`、`rating: 4`、`tags: [烟测]` 并保存，刷新后再打开 | 属性以 YAML 回显，正文不包含 YAML；列表卡片和引用卡片都不展示属性 |
| [ ] | 自定义属性校验和清空 | 在属性区输入嵌套对象或非法 YAML 后保存，再清空 YAML 保存 | 非法内容被拦截并提示原因；清空后 `metadata.properties` 被移除，其他 metadata 字段不丢 |
