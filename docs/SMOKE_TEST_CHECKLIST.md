...
- macOS 原生浏览器 smoke：`bun run smoke:browser-local` 会建立临时 release、SQLite 和服务，不使用 Docker，也拒绝访问常驻 `6676` 服务。它以本机 Chrome 跑桌面/390×844 登录、工作区创建与自动切换、三类笔记创建、笔记编辑与历史、全局搜索，以及根目录/嵌套资源目录创建，要求 console error 与本机 4xx/5xx 均为 0；结束前检查 SQLite 完整性、外键、Workspace/历史/评论/附件/标签关联 orphan 和目录记录，并删除临时数据。
