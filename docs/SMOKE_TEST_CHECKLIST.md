...
| [ ] | 编辑器删除附件 | 编辑已有卡片，删除一张已上传图片并确认，再保存卡片 | 确认后图片立即从编辑器中消失；保存前不调用独立文件删除接口，保存时随笔记、历史和操作日志在同一事务删除；保存失败时文件与数据库关系都恢复 |
...
| [ ] | S3 / OSS 编辑删除 | 编辑卡片删除 S3 图片附件并保存 | 前端立即移除；保存成功后 OSS 对象删除，保存失败时对象恢复，不出现半写 |
...
| [ ] | 自定义路径非空 | 设置如 `qa-prefix/` 后上传测试文件 | 文件保存到指定前缀下 |
