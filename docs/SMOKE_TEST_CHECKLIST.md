...
- 高风险浏览器回归：同一命令还验证字符串/数字/布尔/`null`/数组自定义属性，仅改 metadata 不丢正文、附件与引用，Note/Blinkora/Todo 往返不丢数据，标签与附件组合筛选，附件下载字节一致，以及 Workspace 重命名、默认保护、两卡批量移动和当前 Workspace 级联删除零残留。
- macOS 真实云 S3 smoke：将 `BLINKORA_S3_SMOKE_ENDPOINT`、`BLINKORA_S3_SMOKE_REGION`、`BLINKORA_S3_SMOKE_BUCKET`、`BLINKORA_S3_SMOKE_ACCESS_KEY` 和 `BLINKORA_S3_SMOKE_SECRET_KEY` 写入权限为 `0600` 的本地环境文件；加载后执行 `BLINKORA_S3_SMOKE_CUSTOM_PATH=codex-sqlite-qa bun run smoke:s3-local`。该命令会重新构建隔离原生 release、临时 SQLite 和测试账号，不访问常驻 `6676`；对象只写入专用测试路径下的 `smoke-*` 随机子目录，覆盖真实 S3 配置、`/api/s3file/*` 上传读取、资源移动、对象删除、删除后 404、回退本地存储以及 SQLite 完整性和外键检查。正常结束后会删除临时本地数据与测试对象；不在生产实例运行，也不复用生产附件路径。
