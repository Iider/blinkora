...
function redact(value) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (key.toLowerCase().includes('token') || (typeof item === 'string' && item.startsWith('bkws_'))) {
      out[key] = '<redacted>';
    } else {
      out[key] = redact(item);
    }
...
async function trpc(path, input, token = accountToken, method = 'POST', extraHeaders = {}) {
...
async function main() {
  ...
  assert(health.response.ok && health.json?.status === 'ok', 'health', health.json || health.text);
  pass('health');

  const unauthGuide = await request('/api/agent/mcp-guide.md');
  assert(unauthGuide.response.status === 401, 'agent resource rejects missing token', {
  ...
  const seedA = await trpc('notes.upsert', {
    content: `Agent smoke A seed ${stamp} #agent-smoke-${stamp}`,
    type: 0,
  }, accountToken, 'POST', headersA);
  const seedB = await trpc('notes.upsert', {
    content: `Agent smoke B private ${stamp}`,
    type: 0,
  }, accountToken, 'POST', headersB);
  assert(seedA.workspaceId === workspaceA.id && seedB.workspaceId === workspaceB.id, 'seed workspace ids', { seedA, seedB });
  pass('seed notes');
  ...
  assert(
    createdToken?.id && createdToken?.token?.startsWith('bkws_') && createdToken.workspaceId === workspaceA.id,
  ...
  const listedTokens = await trpc('agentTokens.list', { workspaceId: workspaceA.id });
  assert(
    Array.isArray(listedTokens)
      && listedTokens.some((item) => item.id === createdToken.id && item.token === agentToken)
  ...
  assert(
    guide.response.ok && guide.text.includes('Blinkora MCP / Skill') && guide.text.includes('searchBlinkora'),
  ...
  assert(
    skill.response.ok && skill.text.includes('name: blinkora-workspace') && skill.text.includes('BLINKORA_AGENT_TOKEN'),
  ...
  const wrongWorkspaceHeader = await trpcRaw('notes.list', { page: 1, size: 10 }, agentToken, 'GET', headersB);
  assert(wrongWorkspaceHeader.response.status === 401, 'workspace token rejects mismatched x-workspace-id', {
  ...
  await expectForbidden('agentTokens.list', { workspaceId: workspaceA.id }, agentToken);
  ...
  assert(
    expectedTools.every((name) => toolNames.includes(name)) && !toolNames.includes('workspaces.list'),
  ...
  const flash = await mcp.rpc('tools/call', {
    name: 'upsertBlinkora',
    arguments: { content: `Agent smoke flash ${stamp} #agent-smoke-${stamp}`, type: 'blinkora' },
  });
  const flashNote = flash.result?.structuredContent;
  assert(flashNote?.type === 0 && flashNote.workspaceId === workspaceA.id, 'MCP upsert blinkora', flash);

  const note = await mcp.rpc('tools/call', {
    name: 'upsertBlinkora',
    arguments: { content: `Agent smoke note ${stamp} #agent-smoke-${stamp}`, type: 'note' },
  });
  const noteValue = note.result?.structuredContent;
  assert(noteValue?.type === 1 && noteValue.workspaceId === workspaceA.id, 'MCP upsert note', note);
  ...
  assert(todoValue?.type === 2 && todoValue.workspaceId === workspaceA.id, 'MCP upsert todo', todo);

  const got = await mcp.rpc('tools/call', {
    name: 'getBlinkora',
    arguments: { id: noteValue.id },
  });
  assert(got.result?.structuredContent?.id === noteValue.id, 'MCP getBlinkora', got);

  const updated = await mcp.rpc('tools/call', {
    name: 'updateBlinkora',
    arguments: { id: noteValue.id, content: `Agent smoke updated note ${stamp} #agent-smoke-${stamp}`, type: 1 },
  });
  assert(updated.result?.structuredContent?.content.includes('updated'), 'MCP updateBlinkora', updated);

  const comment = await mcp.rpc('tools/call', {
    name: 'createComment',
    arguments: { noteId: noteValue.id, content: `Agent smoke comment ${stamp}`, kind: 'annotation' },
  });
  const commentValue = comment.result?.structuredContent;
  assert(commentValue?.id && commentValue.noteId === noteValue.id, 'MCP createComment', comment);

  const commentUpdate = await mcp.rpc('tools/call', {
    name: 'updateComment',
    arguments: { id: commentValue.id, content: `Agent smoke comment updated ${stamp}`, status: 'open' },
  });
  assert(commentUpdate.result?.structuredContent?.content.includes('updated'), 'MCP updateComment', commentUpdate);
  ...
  assert(
    comments.result?.structuredContent?.some?.((item) => item.id === commentValue.id),
    'MCP listComments',
    comments,
  );
  ...
  assert(
    searchA.result?.structuredContent?.notes?.some((item) => item.id === seedA.id),
    'MCP search bound workspace',
    searchA,
  );
  const searchB = await mcp.rpc('tools/call', {
    name: 'searchBlinkora',
    arguments: { searchText: `Agent smoke B private ${stamp}`, page: 1, size: 20 },
  });
  assert(
    !searchB.result?.structuredContent?.notes?.some((item) => item.id === seedB.id),
  ...
  const revokedFirst = await trpc('agentTokens.revoke', { id: createdToken.id });
  assert(revokedFirst?.revokedAt, 'revoke first token', revokedFirst);
  const revokedUse = await trpcRaw('notes.list', { page: 1, size: 10 }, agentToken, 'GET');
  assert(revokedUse.response.status === 401, 'revoked first token rejects tRPC', {
  ...
  assert(refreshedToken?.id && refreshedToken?.token?.startsWith('bkws_'), 'refreshed token create', refreshedToken);
  ...
  assert(refreshedTools.result?.tools?.some((tool) => tool.name === 'searchBlinkora'), 'refreshed token MCP works', refreshedTools);
  refreshedMcp.close();
...
try {
  ...
  if (error.details !== undefined) {
    console.error(typeof error.details === 'string' ? error.details : JSON.stringify(redact(error.details), null, 2));
  }
  process.exit(1);
}
