...
router.get(/.*/, async (req, res) => {
  ...
  if (!fullPath.includes('temp/') && !fullPath.endsWith('.bko')) {
    try {
      ...
      const workspaceId = await resolveWorkspaceIdForAccount(Number(token.id), req.headers['x-workspace-id']);
      const isOwner = myFile?.accountId === Number(token.id) ||
        myFile?.note?.accountId === Number(token.id) ||
        token.role === 'superadmin';
      const isSameWorkspace = myFile?.workspaceId === workspaceId ||
        myFile?.note?.workspaceId === workspaceId ||
        token.role === 'superadmin';

      if (myFile && (!isOwner || !isSameWorkspace)) {
        return res.status(401).json({ error: "Unauthorized" });
      }
    } catch (error) {
