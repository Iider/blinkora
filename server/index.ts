...
import backupRouter from './routerExpress/backup';
...
async function bindDevFrontend(app: express.Application, server: any) {
...
async function setupApiRoutes(app: express.Application) {
  ...
  app.use('/api-doc', swaggerUi.serve, swaggerUi.setup(openApiDocument, {
    ...
    customSiteTitle: 'Blinkora API Document',
...
async function bootstrap() {
  try {
    ...
    app.get('/locales/:lng/:ns.json', (req, res) => {
      ...
      if (!/^[A-Za-z-]+$/.test(lng) || !/^[A-Za-z0-9_-]+$/.test(ns)) {
        res.status(400).json({ error: 'Invalid locale path' });
        return;
      }

      const localeFilePath = path.resolve(publicPath, 'locales', lng, `${ns}.json`);
      const localeRoot = path.resolve(publicPath, 'locales');
      if (!localeFilePath.startsWith(localeRoot) || !fs.existsSync(localeFilePath)) {
    ...
    const staticOptions = {
      ...
      setHeaders: (res: express.Response, filePath: string) => {
        ...
        if (ext === 'json' && filePath.includes(`${path.sep}locales${path.sep}`)) {
          res.setHeader('Cache-Control', 'no-store');
          return;
        }
    ...
    if (!server) {
      ...
      await bindDevFrontend(app, server);
    } else {
