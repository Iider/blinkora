...
import { buildS3ObjectKey, createS3ClientFromConfig } from "./s3Storage";
...
export class FileService {
  ...
  public static async getS3Client() {
    ...
    return cache.wrap(`${config.s3Endpoint}-${config.s3Region}-${config.s3Bucket}-${config.s3AccessKeyId}-${config.s3AccessKeySecret}-${config.s3ForcePathStyle}`, async () => {
      const s3ClientInstance = createS3ClientFromConfig(config);
  ...
  static async uploadFile({
    buffer, originalName, type, withOutAttachment = false, accountId, workspaceId, metadata
  }: {
    buffer: Buffer, originalName: string, type: string, withOutAttachment?: boolean, accountId: number, workspaceId?: number, metadata?: any
  }) {
    ...
    if (config.objectStorage === 's3') {
      ...
      const s3Key = buildS3ObjectKey(config.s3CustomPath, timestampedFileName);
  ...
  static async deletePhysicalFile(api_attachment_path: string) {
    const config = await getGlobalConfig({ useAdmin: true });
  ...
  static async deleteFile(api_attachment_path: string) {
    ...
    await this.deletePhysicalFile(api_attachment_path);
    if (attachmentPath) {
  ...
  static async uploadFileStream(
    {
      stream, originalName, fileSize, type, accountId, workspaceId, metadata
    }: {
      stream: ReadableStream, originalName: string, fileSize: number, type: string, accountId: number, workspaceId?: number, metadata?: any
    }) {
    const config = await getGlobalConfig({ useAdmin: true });
    const extension = path.extname(originalName);
    const rawBaseName = path.basename(originalName, extension);
  ...
  static async createAttachment({
    path, name, size, type, noteId, accountId, workspaceId, metadata
  }: {
    path: string, name: string, size: number, type: string, noteId?: number | null, accountId: number, workspaceId?: number, metadata?: any
  }) {
