...
fn list(ctx: ProcedureContext, _input: Value) -> ProcedureFuture {
    async move {
        ...
        let workspace_id = resolve_workspace_id(&ctx, user_id)
            .await
            .unwrap_or_default();
...
fn save_and_validate_s3(ctx: ProcedureContext, input: Value) -> ProcedureFuture {
    async move {
        ...
        let normalized_custom_path = match crate::s3::normalize_custom_path(
            input
                .get("s3CustomPath")
...
async fn resolve_workspace_id_required(
    ...
    let fallback: Option<i32> = sqlx::query_scalar(
