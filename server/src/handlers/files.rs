...
async fn serve_file(
    ...
    let (status, response_bytes, content_range) = match requested_range {
        Some(value) => {
            let parsed = value
                .to_str()
                .ok()
                .and_then(|value| parse_single_byte_range(value, total_length));
            let Some((start, end)) = parsed else {
                return range_not_satisfiable(total_length);
            };
            (
    ...
    let mut response = Body::from(response_bytes).into_response();
    ...
    if let Some(value) = content_range {
        response.headers_mut().insert(
            ...
            HeaderValue::from_str(&value)
                .expect("a generated byte range is always a valid header value"),
        );
    }
...
fn parse_single_byte_range(value: &str, total_length: usize) -> Option<(usize, usize)> {
    if total_length == 0 {
        return None;
    }
    ...
    let end = if end.is_empty() {
        ...
        end.parse::<usize>().ok()?.min(total_length - 1)
    };
    (start <= end).then_some((start, end))
}

fn range_not_satisfiable(total_length: usize) -> Response {
    ...
    *response.status_mut() = StatusCode::RANGE_NOT_SATISFIABLE;
    ...
    response.headers_mut().insert(
        header::CONTENT_RANGE,
        HeaderValue::from_str(&format!("bytes */{total_length}"))
            .expect("a generated byte length is always a valid header value"),
    );
    response
        .headers_mut()
...
mod tests {
    ...
    fn parses_supported_single_byte_ranges() {
        assert_eq!(parse_single_byte_range("bytes=2-5", 10), Some((2, 5)));
        assert_eq!(parse_single_byte_range("bytes=7-", 10), Some((7, 9)));
        assert_eq!(parse_single_byte_range("bytes=-3", 10), Some((7, 9)));
        assert_eq!(parse_single_byte_range("bytes=-30", 10), Some((0, 9)));
        assert_eq!(parse_single_byte_range("bytes=8-30", 10), Some((8, 9)));
    }

    #[test]
    fn rejects_invalid_or_unsupported_byte_ranges() {
        assert_eq!(parse_single_byte_range("bytes=0-1", 0), None);
        assert_eq!(parse_single_byte_range("items=0-1", 10), None);
        assert_eq!(parse_single_byte_range("bytes=1-2,4-5", 10), None);
        assert_eq!(parse_single_byte_range("bytes=10-", 10), None);
        assert_eq!(parse_single_byte_range("bytes=8-7", 10), None);
        assert_eq!(parse_single_byte_range("bytes=-0", 10), None);
        assert_eq!(parse_single_byte_range("bytes=a-b", 10), None);
    }
}
