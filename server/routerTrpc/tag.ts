...
export const tagRouter = router({
  ...
  fullTagNameById: authProcedure
    ...
    .query(async function ({ input, ctx }) {
      ...
      const tag = await prisma.tag.findFirst({ where: { id, accountId, workspaceId } })
      if (!tag) {
      ...
      const getParentTags = async (currentTag: typeof tag): Promise<string[]> => {
        ...
        const parentTag = await prisma.tag.findFirst({
          where: { id: currentTag.parent, accountId, workspaceId }
        });
  ...
  updateTagMany: authProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      const { ids, tag } = input
      const notes = await prisma.notes.findMany({ where: { id: { in: ids }, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) } })
  ...
  updateTagName: authProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      const tag = await prisma.tag.findFirst({
        where: { id, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) }
      })
      if (!tag) throw new Error('Tag not found')
      const tagToNote = await prisma.tagsToNote.findMany({ where: { tagId: id } })
      const noteIds = tagToNote.map(i => i.noteId)
      const hasTagNote = await prisma.notes.findMany({ where: { id: { in: noteIds }, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) } })
      hasTagNote.map(i => {
  ...
  updateTagIcon: authProcedure
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      if (!tag) throw new Error('Tag not found')
      return await prisma.tag.update({ where: { id: tag.id }, data: { icon } })
    }),
  deleteOnlyTag: authProcedure.use(demoAuthMiddleware)
    ...
    .mutation(async function ({ input, ctx }) {
      ...
      for (const noteId of allNotesId) {
        const note = await prisma.notes.findFirst({ where: { id: noteId, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) } })
        if (!note) continue

        const getAllTagIdsInChain = async (tagId: number): Promise<number[]> => {
          const result: number[] = [tagId];
          
          let currentTag = await prisma.tag.findFirst({ where: { id: tagId, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) } });
          while (currentTag && currentTag.parent !== 0) {
            result.push(currentTag.parent);
            currentTag = await prisma.tag.findFirst({ where: { id: currentTag.parent, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) } });
          }

          const childTags = await prisma.tag.findMany({ where: { parent: tagId, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) } });
        ...
        for (const tagId of tagIdsInChain) {
          const tagExists = await prisma.tag.findFirst({
            where: { id: tagId, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) }
          });
          ...
          if (tagExists) {
            const tagUsageCount = await prisma.tagsToNote.count({
              where: {
                tagId,
                note: { accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) }
              }
            });
  ...
  deleteTagWithAllNote: authProcedure.use(demoAuthMiddleware)
    ...
    .mutation(async function ({ input, ctx }) {
      const { id } = input
      const tag = await prisma.tag.findFirst({ where: { id, accountId: Number(ctx.id), workspaceId: Number(ctx.workspaceId) }, include: { tagsToNote: true } })
      const allNotesId = tag?.tagsToNote.map(i => i.noteId) ?? []
