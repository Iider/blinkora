...
export class UserStore implements Store {
  ...
  tokenData = new StorageState<TokenData | null>({ key: 'blinkoraToken', value: null });
  theme: any = 'light';
  ...
  ready(userData: any) {
    ...
    this.isSetup = true;
    eventBus.emit('user:ready', this);
    void this.initializeWorkspaces();
  }

  async initializeWorkspaces() {
    await RootStore.Get(WorkspaceStore).initWorkspaces(this.isLogin);
  }
  ...
  updateBrowserThemeColor(theme: string) {
  ...
  updateAppTitle(customTitle?: string) {
    const appTitle = customTitle?.trim() || 'Blinkora';
  ...
  async initializeSettings(setTheme: (theme: string) => void, i18n: any) {
    const config = await this.blinkora.config.call()
    ...
    if (savedTheme && !this.themeInitialized) {
      this.applyThemePreference(savedTheme, setTheme);
      ...
      try {
        if (config) {
          ...
          if (config.theme && config.theme !== savedTheme) {
            this.applyThemePreference(config.theme, setTheme);
          }
        }
      } catch (error) {
  ...
  use() {
    ...
    useEffect(() => {
      this.updateAppTitle(this.blinkora.config.value?.customTitle);
    }, [this.blinkora.config.value?.customTitle]);
