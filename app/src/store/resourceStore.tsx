...
export class ResourceStore implements Store {
  ...
  loadResources = (folder?: string) => {
    this.clearSelection();
    this.blinkora.resourceList.resetAndCall({
      folder: folder || undefined,
    });
  }

  loadNextPage = () => {
    this.blinkora.resourceList.callNextPage({
  ...
  handleDragEnd = async (result: any) => {
    ...
    const destItem = this.blinkora.resourceList.value?.[destination.index];
    if (!destItem?.isFolder) return;

    const itemsToMove = Array.from(this.selectedItems).map(id =>
      this.blinkora.resourceList.value?.find(item => item.id === Number(id))
    ).filter((item): item is NonNullable<typeof item> => item != null);

    if (itemsToMove.length === 0) {
      const draggedItem = this.blinkora.resourceList.value?.[source.index];
  ...
  handleNewFolder = () => {
    ...
    const currentResources = blinkora.resourceList.value || [];
