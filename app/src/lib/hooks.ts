...
export const useConfigSetting = (configKey: keyof BlinkoraStore['config']['value']) => {
  ...
  useEffect(() => {
    if (blinkora.config.value && blinkora.config.value[configKey]) {
      store.setValue(blinkora.config.value[configKey] as string);
    }
  }, [blinkora.config.value, configKey]);
