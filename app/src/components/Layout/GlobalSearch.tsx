...
import { allSettingMetas } from '@/pages/settingsMeta';
import { BlinkoraCard } from '../BlinkoraCard';
import { ConvertTypeButton } from '../BlinkoraCard/cardFooter';
...
export const GlobalSearch = observer(({ isOpen, onOpenChange }: GlobalSearchProps) => {
  ...
  const store = RootStore.Local(() => ({
    ...
    setSearchQuery(value: string) {
      ...
      if (value) {
        ...
        clearSearchState(blinkoraStore);
        clearSearchFiltersFromUrl();
        blinkoraStore.noteList.resetAndCall({ page: 1, size: 20 });
        blinkoraStore.resourceList.resetAndCall({
          page: 1,
          size: 20,
          searchText: '',
          folder: undefined,
  ...
  useEffect(() => {
    if (isOpen) {
      store.searchQuery = blinkoraStore.searchText || '';
  ...
  const navigateToResource = (resource: ResourceType) => {
    //download
    downloadFromLink(getBlinkoraEndpoint(resource.path));
    onOpenChange(false);
  };
