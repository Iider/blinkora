...
export async function getTokenData(): Promise<TokenData | null> {
  try {
    ...
    const response = await fetch(getBlinkoraEndpoint('/api/auth/profile'), {
      headers: {
...
export async function signIn(
  ...
  try {
    ...
    if (provider === 'two-factor') {
      const response = await fetch(getBlinkoraEndpoint('/api/auth/verify-2fa'), {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
        },
...
export async function signOut(options: { redirect?: boolean; callbackUrl?: string } = {}): Promise<{ url: string }> {
  try {
    ...
    await fetch(getBlinkoraEndpoint('/api/auth/logout'), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
