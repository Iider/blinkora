        ...
        if (part.startsWith('#') && part.length > 1 && part.match(helper.regex.isContainHashTag)) {
          return (
            <span key={`${lineIndex}-${index}`}
              className="select-none blinkora-tag px-1 font-bold cursor-pointer hover:opacity-80 !transition-all"
              onClick={async () => {
...
export const MarkdownRender = observer(({ content = '', onChange, largeSpacing = false }: { content?: string, onChange?: (newContent: string) => void, largeSpacing?: boolean }) => {
