...
const AttachmentsRender = observer((props: IProps) => {
  ...
  return (
    <div className={`flex flex-col ${files.length == 0 ? 'gap-[2px]' : 'gap-[4px]'}`}>
      ...
      {/* audio render */}
      <AudioRender files={files} preview={preview} onDelete={props.onDelete} />
      ...
      <DraggableFileGrid
        ...
        renderItem={(file) => (
          ...
          >
            <FileIcons path={file.name} isLoading={!preview && !!file.uploadPromise?.loading?.value} />
            <div className='truncate text-xs md:text-sm font-bold'>{file.name}</div>
            {!file.uploadPromise?.loading?.value && !preview &&
              <DeleteIcon className='ml-auto group-hover:opacity-100 opacity-0' files={files} file={file} onDeleted={props.onDelete} />
            }
          </div>
        )}
      />
...
const FilesAttachmentRender = observer(({
  ...
  const handleDelete = (file: FileType) => {
    const targetPath = file.uploadPromise?.value || file.preview;
    setFiles(current => current.filter(item => {
      const itemPath = item.uploadPromise?.value || item.preview;
      if (targetPath && itemPath) return itemPath !== targetPath;
