...
export const ResourceReferenceButton = observer(({ store }: Props) => {
  ...
  useEffect(() => {
    if (isOpen) {
      blinkora.resourceList.resetAndCall({ folder: undefined });
    }
  }, [isOpen]);

  useEffect(() => {
    if (isOpen && searchText) {
      blinkora.resourceList.resetAndCall({ searchText, folder: undefined });
    }
  }, [searchText, isOpen]);
      ...
      <PopoverContent className="w-[300px] sm:w-[300px]">
        <div className="p-1 w-full">
          ...
          <ScrollArea
            className="h-[300px] "
            onBottom={() => {
              blinkora.resourceList.callNextPage({});
            }}
          >
            <LoadingAndEmpty isLoading={blinkora.resourceList.loading.value} isEmpty={blinkora.resourceList.value?.length === 0} />
            <PhotoProvider>
              <div className="space-y-2 w-full">
                {blinkora.resourceList.value?.map((attachment) => (
