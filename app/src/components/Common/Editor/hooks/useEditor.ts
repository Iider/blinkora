...
import { getBlinkoraEndpoint, stripBlinkoraFileAccessTokensFromText, withBlinkoraFileAccessToken } from '@/lib/blinkoraEndpoint';
...
const getVditorCdn = (): string => {
  return getBlinkoraEndpoint('/vditor-assets').replace(/\/$/, "");
};
...
const renderAllVditorContent = (
  ...
  // Lazy load images
  addAccessTokenToProtectedImages(editorElement);
...
export const useEditorInit = (
  ...
  useEffect(() => {
    ...
    const vditor = new Vditor("vditor" + "-" + mode, {
      ...
      input: (value) => {
        onChange?.(stripBlinkoraFileAccessTokensFromText(value))
      ...
      upload: {
        url: getBlinkoraEndpoint('/api/file/upload'),
        success: (editor, res) => {
        ...
        headers: {
          ...
          ...(RootStore.Get(WorkspaceStore).workspaceId ? {
            'x-workspace-id': String(RootStore.Get(WorkspaceStore).workspaceId)
          } : {})
        },
        ...
        fieldName: 'file',
        multiple: false,
        linkToImgUrl: getBlinkoraEndpoint('/api/file/upload-by-url'),
        linkToImgFormat(res) {
          const data = JSON.parse(res)
          const result = {
            msg: '',
  ...
  }, [mode, blinkora.config.value?.toolbarVisibility, store.viewMode, isPc]);
  ...
  useEffect(() => {
    if (mode == 'create') {
      if (searchParams.get('path') == 'notes') {
        ...
        store.noteType = NoteType.BLINKORA
      }
