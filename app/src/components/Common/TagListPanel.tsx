...
const CustomIcon = observer(({ onSubmit }: { onSubmit: (icon: string) => void }) => {
  ...
  return <div className='w-full flex flex-col gap-2'>
    <Input
      label={t('custom-icon')}
      placeholder={t('enter-custom-icon')}
      value={icon}
      onValueChange={setIcon}
      description={<>
        {t('blinkora-custom-icon-tip')} <a className="text-blue-500" href="https://icon-sets.iconify.design/" target="_blank">Iconify</a>
      </>}
    />
    <div className="flex justify-end">
      <Button color="primary" onPress={() => { onSubmit(icon) }}>{t('submit')}</Button>
    </div>
  </div>
})
...
export const TagListPanel = observer(() => {
  ...
  const isSelected = (id) => {
    return blinkora.noteListFilterConfig.tagId == id && searchParams.get('path') == 'all'
  }
  useEffect(() => { }, [blinkora.noteListFilterConfig.tagId])
  return (
    <>
      ...
      <TreeView
        ...
        data={flattenTree({
          name: "",
          children: blinkora.tagList.value?.listTags,
        })}
        ...
        nodeRenderer={({
          ...
          <div {...getNodeProps()} style={{ paddingLeft: 20 * (level - 1) + 6 }} >
            <div className={`${SideBarItem}relative group ${(isSelected(element.id)) ? '!bg-primary !text-primary-foreground' : ''}`}
              onClick={e => {
                //@ts-ignore
                base.currentRouter = blinkora.allTagRouter
                blinkora.updateTagFilter(Number(element.id))
            ...
            >
              ...
              <Dropdown>
                ...
                <DropdownMenu aria-label="Static Actions">
                  <DropdownItem key="customIcon" onPress={async () => {
                    if (!isPc) {
                  ...
                  <DropdownItem key="moveUp" onPress={async () => {
                    ...
                    const siblings = findSiblings(blinkora.tagList.value?.listTags || [], element.id as number);
                    if (siblings) {
