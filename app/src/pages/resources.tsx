...
import { MemoizedResourceItem } from "@/components/BlinkoraResource/ResourceItem";
import { ResourceMultiSelectPop } from "@/components/BlinkoraResource/ResourceMultiSelectpop";
...
const Page = observer(() => {
  ...
  const resources = useMemo(() => {
    const allResources = toJS(resourceStore.blinkora.resourceList.value) || [];
    // Filter out .folder placeholder files
    return allResources.filter(resource => resource.name !== '.folder');
  }, [resourceStore.blinkora.resourceList.value]);
    ...
    <>
      <DragDropContext onDragEnd={resourceStore.handleDragEnd}>
        ...
        >
                  ...
                  >
                    <Breadcrumbs variant="solid" className="ml-[-8px]" size='lg'>
                      {folderBreadcrumbs.map((folder, index) => (
                        ...
                        >
                          {folder === 'Root' ? t('resource-root') : folder}
                        </BreadcrumbItem>
                      ))}
                    </Breadcrumbs>
                  </motion.div>
          ...
          <LoadingAndEmpty
            isLoading={resourceStore.blinkora.resourceList.isLoading}
